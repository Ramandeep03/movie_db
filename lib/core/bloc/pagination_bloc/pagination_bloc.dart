...
abstract class PaginationBloc<T> extends Bloc<PaginationEvent, PaginationState<T>> {
  ...
  Future<void> _onFetch(PaginationFetch event, Emitter<PaginationState<T>> emit) async {
    _page = 1;
    _hasReachedEnd = false;
    emit(PaginationInitialPageLoading<T>());

    try {
      final items = await fetchItems(page: _page, limit: pageSize, filters: event.filters);
      _hasReachedEnd = items.length < pageSize;

      emit(PaginationLoaded<T>(items: items, newItems: items, hasReachedEnd: _hasReachedEnd));
  ...
  Future<void> _onFetchNext(PaginationFetchNext event, Emitter<PaginationState<T>> emit) async {
  ...
  Future<void> _onRefresh(PaginationRefresh event, Emitter<PaginationState<T>> emit) async {
